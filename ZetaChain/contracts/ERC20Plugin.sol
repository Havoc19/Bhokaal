...
abstract contract ERC20Plugins is ERC20, IERC20Plugins, ReentrancyGuardExt {
    ...
    function pluginBalanceOf(
        address plugin,
        address account
    ) public view virtual nonReentrantView(_guard) returns (uint256) {
    ...
    function _addPlugin(address account, address plugin) internal virtual {
        ...
        if (IPlugin(plugin).token() != IERC20Plugins(address(this)))
            revert InvalidTokenInPlugin();
        if (!_plugins[account].add(plugin)) revert PluginAlreadyAdded();

        if (_plugins[account].length() > maxPluginsPerAccount)
            revert PluginsLimitReachedForAccount();
    ...
    function _afterTokenTransfer(
        ...
        address to,
        uint256 amount
    ) internal virtual override nonReentrant(_guard) {
